...
#[test]
fn test_predict() {
    assert_eq!(predict(&[]), None);
    assert_eq!(predict(&[1]), None);
    assert_eq!(predict(&[0, 0]), Some(0));
    assert_eq!(predict(&[0, 2, 4, 6]), Some(8));
    assert_eq!(predict(&[3, 3, 5, 9, 15]), Some(23));
...
fn part_1(s: &str) -> Num {
    ...
    histories
        .into_iter()
        .filter_map(|history| predict(&history))
        .sum()
}
